...
module.exports=async(req,res)=>{
  try{
    ...
    if(method==='GET'){
      const {data,error}=await supabase.from('tokens').select('used').eq('token',token).maybeSingle();
    ...
    if(method==='POST'){
      const {data,error}=await supabase.from('tokens').update({used:true,used_at:new Date().toISOString()}).eq('token',token).eq('used',false).select('token').maybeSingle();
      if(error)return res.status(500).json({ok:false,reason:'server_error'});
  ...
  }catch(e){return res.status(500).json({ok:false,reason:'server_crash'});}
};
