...
const factors = {
  "躯体化": { indices: [0, 3, 11, 26, 39, 46, 50, 56, 68, 76], explanation: "反映主观的身体不适感，包括心血管、胃肠道、呼吸系统等不适，以及头痛、背痛、肌肉酸痛等躯体表现" },
  "强迫症状": { indices: [1, 2, 8, 15, 27, 36, 44, 53, 62, 74], explanation: "反映那些明知没有必要，但又无法摆脱的无意义的思想、冲动和行为" },
  "人际关系敏感": { indices: [5, 20, 33, 35, 37, 41, 60, 64, 69, 78], explanation: "反映与他人相处时的不自在感和自卑感，尤其是在与他人相比较时更突出" },
  "抑郁": { indices: [4, 13, 14, 28, 29, 30, 38, 52, 63, 71, 79, 85, 88], explanation: "反映与临床抑郁症相联系的广泛的概念，包括忧郁、兴趣减退、失望、悲观等" },
  "焦虑": { indices: [10, 22, 31, 33, 34, 42, 55, 65, 67, 80, 86], explanation: "反映通常与临床明显焦虑相联系的症状和体验，如紧张、神经过敏、无法静坐等" },
  "敌对": { indices: [6, 21, 23, 40, 58, 70, 81, 84, 89], explanation: "主要从思想、感情及行为三个方面来反映敌对的表现，包括厌烦、争论、摔物等" },
  "恐怖": { indices: [12, 24, 45, 47, 49, 59, 61, 72, 77, 82], explanation: "反映对孤独和公共场合的惧怕，包括出门旅行、空旷场地、人群、交通工具等" },
  "偏执": { indices: [7, 16, 17, 32, 34, 51, 57, 66, 75, 83], explanation: "反映偏执性思维的基本特征，如投射性思维、敌对、猜疑、关系观念等" },
  "精神病性": { indices: [9, 18, 43, 48, 54, 62, 73, 79, 84, 87], explanation: "反映各式各样的急性症状和行为，如幻听、思维播散、被控制感等" },
  "其他(睡眠、饮食等)": { indices: [19, 25, 39, 48, 55, 60, 64, 71, 79], explanation: "反映睡眠及饮食情况，如入睡困难、早醒、多梦、食欲不振、饮食过量等" }
};
...
function getTokenFromUrl(){ try{ const u=new URL(location.href); return u.searchParams.get('t')||'';}catch(e){return '';} }
async function api(path,opts){ const r=await fetch(path,opts); try{return await r.json();}catch(e){return {ok:false,reason:'server_error'};} }
...
function displayResults() {
  ...
  for (const factor in factorScores) {
    ...
    if (score < 1.5) { level = '正常'; levelClass = 'level-normal'; }
    else if (score < 2.5) { level = '轻度'; levelClass = 'level-mild'; }
    else if (score < 3.5) { level = '中度'; levelClass = 'level-moderate'; }
    else { level = '重度'; levelClass = 'level-severe'; }
