...
// === Token: one-time link support (Vercel + Supabase) ===
let __token_ok__ = false;
let __token_value__ = null;

function __getTokenFromUrl() {
  try {
...
async function __checkToken(t) {
  const res = await fetch(`/api/token?token=${encodeURIComponent(t)}`);
  return res.json(); // {ok:true}|{ok:false,reason}
}

async function __consumeToken(t) {
  try {
    await fetch(`/api/token?token=${encodeURIComponent(t)}`, { method: 'POST' });
  } catch(e) {}
}

// 载入即校验
document.addEventListener('DOMContentLoaded', async () => {
  __token_value__ = __getTokenFromUrl();
  const startBtn = document.getElementById('start-test');
  if (!__token_value__) {
    __token_ok__ = false;
    if (startBtn) startBtn.disabled = true;
    alert('访问参数缺失，请从正确链接进入。');
    return;
  }
  try {
    const chk = await __checkToken(__token_value__);
    if (chk && chk.ok) {
      __token_ok__ = true;
      ...
      __token_ok__ = false;
      if (startBtn) startBtn.disabled = true;
      alert(chk.reason === 'used' ? '此链接已被使用' : '链接无效');
    }
  } catch(e) {
    if (startBtn) startBtn.disabled = true;
    alert('网络异常，请稍后再试。');
        ...
        // 开始测试
        function startTest() { if(!__token_ok__){ alert('链接不可用或已被使用'); return; }
            startTime = new Date();
        ...
        function showResults() {
            ...
            calculateResults();
            displayResults(); __consumeToken(__token_value__); }
